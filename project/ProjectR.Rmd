...
of how it declines from the maximum donation to the 99%. 995 is about 1/3 of the maximum.    Lets see the curve for the top 5% even if we won't be able to make out their names
```{r 11top5}
...
Th reductio is leveling off or as you can see there are more donations at $40K to $50K range 
...
between tot_rec and cas_on_han and the other factors.  Looking at the box plots at the intersection of cas_on_han and com_typ you can see a clear delineation.  The graph and scale are small but seems a similar dilineation for tot_rec as well.  
...
group of Super PACs.
```{r 17best1,fig.width=12, fig.height=12,}
# Filter to the Top 5% committees
upperCampComm <- campComm[campComm$tot_rec > quantile(campComm$tot_rec,probs=.95),]
# condense rows to just plottable data
sumUpperCampComm <- 
...
ggplot(aes(x=com_nam,y=tot_rec,label=tot_rec),
  ...
  theme(axis.text.x = element_text(colour="grey20",size=15,angle=0,hjust=.5,vjust=.5,face="plain"),
    axis.text.y = element_text(colour="grey20",size=6,angle=0,hjust=.5,
                               vjust=.5,face="plain"),
